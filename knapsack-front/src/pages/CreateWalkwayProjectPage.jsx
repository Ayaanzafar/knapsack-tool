...
export default function CreateWalkwayProjectPage() {
  ...
  // Load existing walkway projects when tab is active
  useEffect(() => {
    if (activeTab === 'existing') loadProjects();
  }, [activeTab, currentPage, sortBy, debouncedSearch]);
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      console.error('Failed to load walkway projects:', err);
  ...
  const handleCreateSubmit = async (e) => {
    e.preventDefault();
    setError('');
    if (!clientName.trim() || !projectId.trim()) {
    ...
    try {
      ...
      localStorage.setItem(WALKWAY_PROJECT_KEY, project.id.toString());
      navigate('/walkway-app');
    } catch (err) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8 animate-fade-in">
          ...
          <p className="text-gray-700 text-lg font-medium">
            Create or access your walkway projects
          </p>
        </div>

        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl overflow-visible border border-yellow-200/50">
          {/* Tabs */}
          <div className="rounded-t-3xl overflow-hidden">
            <div className="flex border-b border-yellow-200/50 bg-linear-to-r from-yellow-50/50 to-amber-50/50">
              {[
                { key: 'new', label: 'Create New Project', icon: 'M12 4v16m8-8H4' },
                { key: 'existing', label: 'Open Existing Project', icon: 'M5 19a2 2 0 01-2-2V7a2 2 0 012-2h4l2 2h4a2 2 0 012 2v1M5 19h14a2 2 0 002-2v-5a2 2 0 00-2-2H9a2 2 0 00-2 2v5a2 2 0 01-2 2z' }
              ].map(tab => (
                <button
                  key={tab.key}
                  onClick={() => setActiveTab(tab.key)}
                  className={`flex-1 py-5 text-center text-base font-bold transition-all duration-300 relative overflow-hidden group ${activeTab === tab.key ? 'text-white' : 'text-gray-600 hover:text-gray-900'}`}
                >
                  <span className={`absolute inset-0 transition-all duration-300 ${activeTab === tab.key ? 'bg-linear-to-r from-yellow-500 to-amber-500 opacity-100' : 'bg-linear-to-r from-yellow-400 to-amber-400 opacity-0 group-hover:opacity-10'}`} />
                  ...
                  {activeTab === tab.key && <span className="absolute bottom-0 left-0 right-0 h-1 bg-linear-to-r from-yellow-500 to-amber-500" />}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="px-8 py-10">
            {error && (
              <div className="mb-6 bg-linear-to-r from-red-50 to-rose-50 border-l-4 border-red-500 text-red-700 p-4 rounded-xl shadow-sm">
            ...
            {activeTab === 'new' ? (
              ...
              <form onSubmit={handleCreateSubmit} className="space-y-7">
                <div className="text-center mb-8">
                  ...
                  <p className="text-gray-600 mt-2">Fill in the information below to start your walkway calculation</p>
                </div>

                <div className="space-y-6">
                  <div>
                    <label htmlFor="clientName" className="text-sm font-bold text-gray-700 mb-2 flex items-center gap-2">
                  ...
                  <div>
                    <label htmlFor="projectId" className="block text-sm font-bold text-gray-700 mb-2">
                      Project ID <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text" id="projectId" required
                  ...
                  <div>
                    <label htmlFor="projectName" className="text-sm font-bold text-gray-700 mb-2 flex items-center gap-2">
                      <svg className="w-4 h-4 text-yellow-600" fill="currentColor" viewBox="0 0 20 20">
              ...
              <div className="space-y-6">
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="relative flex-1">
                    ...
                    {searchTerm && (
                      <button onClick={() => setSearchTerm('')} className="absolute right-4 top-3.5 text-gray-400 hover:text-gray-600 transition-colors p-0.5 rounded-full hover:bg-gray-100">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                      </button>
                  ...
                  <select
                    value={sortBy} onChange={(e) => { setSortBy(e.target.value); setCurrentPage(1); }}
                ...
                {!listLoading && totalProjects > 0 && (
                  <div className="text-sm font-medium text-gray-600 text-center bg-yellow-50/50 rounded-lg py-2 px-4">
                    Showing <span className="font-bold text-yellow-600">{startIndex}-{endIndex}</span> of <span className="font-bold text-yellow-600">{totalProjects}</span> walkway projects
                  </div>
                )}
                ...
                {listLoading ? (
                  ...
                  <div className="text-center py-16 bg-linear-to-br from-yellow-50/50 to-amber-50/50 rounded-2xl border-2 border-dashed border-yellow-300">
                    ...
                    <p className="text-gray-600 font-medium text-lg">
                      {searchTerm ? 'No walkway projects found matching your search.' : 'No walkway projects found.'}
                    </p>
                    <p className="text-gray-500 text-sm mt-2">
                      {searchTerm ? 'Try adjusting your search terms.' : 'Create your first walkway project to get started!'}
                    </p>
                  </div>
                ) : (
                  <>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-sm text-gray-500 flex flex-wrap gap-x-4 gap-y-1 mt-3">
                                ...
                                <span className="flex items-center gap-1.5">
                                  ...
                                  <span className="font-medium">{project.projectId || 'N/A'}</span>
                                </span>
                                <span className="text-gray-300">•</span>
                                <span className="flex items-center gap-1.5">
                                  <span className="text-gray-400 text-xs">Updated:</span>
                                  {new Date(project.updatedAt).toLocaleString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true })}
                                </span>
                              </div>
                            </div>
                            <div className="shrink-0 w-10 h-10 rounded-xl bg-linear-to-br from-yellow-100 to-amber-100 flex items-center justify-center group-hover:from-yellow-500 group-hover:to-amber-500 transition-all duration-300 group-hover:scale-110">
                    ...
                    {totalPages > 1 && (
                      <div className="flex justify-center items-center gap-4 pt-4">
                        <button
                          ...
                          className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all duration-300 border-2 shadow-sm ${currentPage === 1 ? 'bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed' : 'bg-linear-to-r from-yellow-500 to-amber-500 border-transparent text-white hover:from-yellow-600 hover:to-amber-600 hover:shadow-lg transform hover:-translate-y-0.5'}`}
                        >
                        ...
                        <button
                          ...
                          className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all duration-300 border-2 shadow-sm ${currentPage === totalPages ? 'bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed' : 'bg-linear-to-r from-yellow-500 to-amber-500 border-transparent text-white hover:from-yellow-600 hover:to-amber-600 hover:shadow-lg transform hover:-translate-y-0.5'}`}
                        >
                          Next
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 5l7 7-7 7" />
